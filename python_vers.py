...
def genotype_VCF2hdf5(data_path, donor_id, chromosome, save_path, study_name):
    ...
    # Read the VCF file header to get column names
    columns = []
    ...
    # Read the VCF file and assign columns
    df = pd.read_csv(data_path, sep='\t', comment='#', compression='gzip', header=None)
    df.columns = columns
    genotype_cols = columns[9:]  # Genotype columns start from the 10th column onwards

    df_genotypes = df[genotype_cols]
    
    # Ensure the genotypes are phased
    phased_genotypes = df_genotypes.applymap(lambda x: x if isinstance(x, str) and '|' in x else None)
    
    data = phased_genotypes.to_numpy()
    ...
    with h5py.File(tmp_h5_file, 'a') as h5_gen_file:
        ...
        if 'genotype' in group:
            del group['genotype']
        group.create_dataset('genotype', data=data, compression='gzip', compression_opts=5, chunks=True)
...
if __name__ == "__main__":
    ...
    if args.flag == 'individuals':
        ...
        # Merge temporary HDF5 files into the final HDF5 file
    ...
    # # Verification step to check the structure and data in the HDF5 file
    # h5_file = os.path.join(args.outdir, f'{args.cohort_name}.h5')
    # verify_hdf5_structure(h5_file)
